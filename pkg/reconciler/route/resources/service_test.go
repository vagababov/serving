	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !cmp.Equal(tt.expectedLabels, got.Labels) {
				t.Errorf("Unexpected Labels (-want +got): ", cmp.Diff(tt.expectedLabels, got.Labels))
			}
			if !cmp.Equal(tt.expectedAnnos, got.ObjectMeta.Annotations) {
				t.Errorf("Unexpected Annotations (-want +got): ", cmp.Diff(tt.expectedAnnos, got.ObjectMeta.Annotations))
			}
			if !cmp.Equal(tt.expectedSpec, got.Spec) {
				t.Errorf("Unexpected ServiceSpec (-want +got): ", cmp.Diff(tt.expectedSpec, got.Spec))
			}
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := GetNames(tt.services); !cmp.Equal(got, tt.want) {
				t.Errorf("GetNames() (-want, +got) = ", cmp.Diff(tt.want, got))
			}
		})
	}
}
