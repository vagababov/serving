...
// controller reconciling v1.Route.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.Route. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.Route) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Route.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.Route. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Route) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Route if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Route.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servingv1.RouteLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.Route, desired *v1.Route) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			...
			getter := r.Client.ServingV1().Routes(desired.Namespace)
		...
		updater := r.Client.ServingV1().Routes(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Route) (*v1.Route, error) {

	getter := r.Lister.Routes(resource.Namespace)
	...
	patcher := r.Client.ServingV1().Routes(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Route) (*v1.Route, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Route, reconcileEvent reconciler.Event) (*v1.Route, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
