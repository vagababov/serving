...
// controller reconciling v1.Service.
...
// controller finalizing v1.Service.
type Finalizer interface {
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Service) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Service if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Service.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.Service) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Service if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.Service.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.Service) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.Service) reconciler.Event
...
type reconcilerImpl struct {
	...
	Lister servingv1.ServiceLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servingv1.ServiceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Services(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.Service, desired *v1.Service) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ServingV1().Services(desired.Namespace)
		...
		updater := r.Client.ServingV1().Services(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Service) (*v1.Service, error) {

	getter := r.Lister.Services(resource.Namespace)
	...
	patcher := r.Client.ServingV1().Services(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Service) (*v1.Service, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Service, reconcileEvent reconciler.Event) (*v1.Service, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
