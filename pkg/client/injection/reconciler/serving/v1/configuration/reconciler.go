...
// controller reconciling v1.Configuration.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.Configuration. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.Configuration) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Configuration.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.Configuration. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Configuration) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Configuration if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Configuration.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.Configuration) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Configuration if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.Configuration.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.Configuration) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.Configuration) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.Configuration resources.
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Configurations(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.Configuration, desired *v1.Configuration) error {
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Configuration) (*v1.Configuration, error) {

	getter := r.Lister.Configurations(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Configuration) (*v1.Configuration, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Configuration, reconcileEvent reconciler.Event) (*v1.Configuration, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
