...
// controller reconciling v1.Revision.
type Interface interface {
	...
	ReconcileKind(ctx context.Context, o *v1.Revision) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Revision.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.Revision. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.Revision) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.Revision if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.Revision.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.Revision) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.Revision if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.Revision.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.Revision) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.Revision) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.Revision resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servingv1.RevisionLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Revisions(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.Revision, desired *v1.Revision) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ServingV1().Revisions(desired.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.Revision) (*v1.Revision, error) {

	getter := r.Lister.Revisions(resource.Namespace)
	...
	patcher := r.Client.ServingV1().Revisions(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.Revision) (*v1.Revision, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.Revision, reconcileEvent reconciler.Event) (*v1.Revision, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
