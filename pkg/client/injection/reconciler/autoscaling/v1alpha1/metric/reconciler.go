...
// controller reconciling v1alpha1.Metric if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Metric.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Metric) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Metric if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.Metric.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.Metric) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.Metric) reconciler.Event
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Metrics(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.Metric, desired *v1alpha1.Metric) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.AutoscalingV1alpha1().Metrics(desired.Namespace)
		...
		updater := r.Client.AutoscalingV1alpha1().Metrics(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Metric) (*v1alpha1.Metric, error) {

	getter := r.Lister.Metrics(resource.Namespace)
	...
	patcher := r.Client.AutoscalingV1alpha1().Metrics(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Metric) (*v1alpha1.Metric, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Metric, reconcileEvent reconciler.Event) (*v1alpha1.Metric, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
