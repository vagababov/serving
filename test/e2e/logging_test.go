...
func TestRequestLogs(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Fail to get ConfigMap config-observability: ", err)
	}
	...
	if err != nil {
		t.Fatalf("Fail to fetch the pod: ", err)
	}
	...
	if err := waitForLog(t, clients, pod.Namespace, pod.Name, "queue-proxy", func(log logLine) bool {
		...
		t.Fatalf("Got error waiting for normal request logs: ", err)
	}
	...
	if strings.EqualFold(cm.Data["logging.enable-probe-request-log"], "true") {
		...
		if err := waitForLog(t, clients, pod.Namespace, pod.Name, "queue-proxy", func(log logLine) bool {
			...
			t.Fatalf("Got error waiting for health check log: ", err)
		}
	} else {
