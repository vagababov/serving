...
func runtimeRequestWithExpectations(t *testing.T, client *http.Client, url string,
	...
	if err != nil {
		if !allowDialError || !isDialError(err) {
			t.Errorf("Error making GET request: ", err)
		}
		return nil
	}
	...
	for _, e := range responseExpectations {
		if err := e(resp); err != nil {
			t.Errorf("Error meeting response expectations: ", err)
			dumpResponse(t, resp)
			return nil
		}
	}
	...
	if resp.StatusCode == http.StatusOK {
		...
		if err != nil {
			t.Errorf("Unable to read response body: ", err)
			dumpResponse(t, resp)
			return nil
		}
		ri := &types.RuntimeInfo{}
		if err := json.Unmarshal(b, ri); err != nil {
			t.Errorf("Unable to parse runtime image's response payload: ", err)
			return nil
		}
		return ri
	}
...
func dumpResponse(t *testing.T, resp *http.Response) {
	...
	if err != nil {
		t.Errorf("Error dumping response: ", err)
	}
	t.Log(string(b))
}
