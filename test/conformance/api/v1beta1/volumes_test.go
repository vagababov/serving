...
func TestConfigMapVolume(t *testing.T) {
	...
	test.EnsureCleanup(t, func() {
		...
		if err := clients.KubeClient.Kube.CoreV1().ConfigMaps(test.ServingNamespace).Delete(context.Background(), configMap.Name, metav1.DeleteOptions{}); err != nil {
			t.Errorf("ConfigMaps().Delete() = ", err)
		}
	})
...
func TestProjectedConfigMapVolume(t *testing.T) {
	...
	// Setup initial Service
	if _, err := v1b1test.CreateServiceReady(t, clients, &names, withVolume); err != nil {
		t.Fatalf("Failed to create initial Service %v: %v", names.Service, err)
	}
...
func TestProjectedSecretVolume(t *testing.T) {
	...
	// Setup initial Service
	if _, err := v1b1test.CreateServiceReady(t, clients, &names, withVolume, withSubpath); err != nil {
		t.Fatalf("Failed to create initial Service %v: %v", names.Service, err)
	}
