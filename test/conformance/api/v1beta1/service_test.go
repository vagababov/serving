...
func TestServiceCreateAndUpdate(t *testing.T) {
	...
	if objects.Service, err = v1b1test.PatchService(t, clients, objects.Service, rtesting.WithServiceTemplateMeta(metadata)); err != nil {
...
func waitForDesiredTrafficShape(t *testing.T, sName string, want map[string]v1.TrafficTarget, clients *test.Clients) error {
	return v1b1test.WaitForServiceState(
		clients.ServingBetaClient, sName, func(s *v1beta1.Service) (bool, error) {
			// IsServiceReady never returns an error.
			if ok, _ := v1b1test.IsServiceReady(s); !ok {
				return false, nil
			}
...
func TestServiceBYOName(t *testing.T) {
	...
	// Setup initial Service
	objects, err := v1b1test.CreateServiceReady(t, clients, &names, func(svc *v1beta1.Service) {
...
func TestServiceWithTrafficSplit(t *testing.T) {
	...
	objects.Service, err = v1b1test.UpdateServiceRouteSpec(t, clients, names, v1.RouteSpec{
	...
	if objects.Service, err = v1b1test.PatchService(t, clients, objects.Service, rtesting.WithServiceImage(releaseImagePath2)); err != nil {
	...
	if objects.Service, err = v1b1test.PatchService(t, clients, objects.Service, rtesting.WithServiceImage(releaseImagePath3)); err != nil {
